...
export class CommandExecutor {
  ...
  private async runStep(step: CmdStep): Promise<void> {
    ...
    logger.step(this.url, `${action} ${args.join(' ')}`, { mode: 'cmd', step: action })
  ...
  private async goto(args: string[]) {
    ...
    logger.step(this.url, `GOTO ${url}`, { mode: 'cmd', step: 'GOTO', url })
  }
  ...
  private async scan(args: string[]) {
    ...
    logger.highlight(this.url, { type: 'scan', selector, count: found.length })
    logger.step(this.url, `SCAN found ${found.length} elements matching "${selector}"`, { mode: 'cmd', step: 'SCAN' })
  }
  ...
  private async extract(args: string[]) {
    ...
    logger.highlight(this.url, { selector, attr, count: data.length })
  }
  ...
  private async repeat(step: CmdStep) {
    ...
    const elements = await this.page.$$(selector)
    logger.step(this.url, `REPEAT ${elements.length}x over "${selector}"`, { mode: 'cmd', step: 'REPEAT' })

    for (let i = 0; i < elements.length; i++) {
      logger.step(this.url, `  REPEAT iteration ${i + 1}/${elements.length}`, { mode: 'cmd' })
  ...
  private async loop(step: CmdStep) {
    ...
    for (let i = 0; i < count; i++) {
      logger.step(this.url, `  LOOP iteration ${i + 1}/${count}`, { mode: 'cmd' })
      for (const child of step.children) {
        await this.runStep(child)
      }
    }
